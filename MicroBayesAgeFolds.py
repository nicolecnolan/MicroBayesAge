...
def processFold(nFold):
    ...
    sTrainFileName = str(nFold) + sPrefix + "_train_DNAm_matrix.pickle"
    sTestFileName  = str(nFold) + sPrefix + "_test_DNAm_matrix.pickle"
    ...
    merged_df = pd.merge(test_DNAm_df, full_prediction_df, left_on="SampleID", right_on="Sample", how="inner")
    predictions_df = merged_df[["Sample", "Age_x", "Age_y"]]
    predictions_df.rename(columns={"Age_x": "Real_Age", "Age_y": "Predicted_Age_1"}, inplace=True)
    ...
    predictions_df = predictions_df.merge(combined_prediction_df, on='Sample')
    predictions_df.rename(columns={"Age": "Predicted_Age_2"}, inplace=True)
...
sOutputFileName = sPrefix + "Predictions_SplitAge_" + str(split_age) + ".pickle"
